...
<body>  
    ...
    <script>  
        ...
        function moveButton() {  
            ...
            const x = Math.random() * (window.innerWidth - noBtn.offsetWidth - 40) + 20; // Ajout de marges  
            const y = Math.random() * (window.innerHeight - noBtn.offsetHeight - 40) + 20; // Ajout de marges  
            ...
            yesBtn.style.padding = `${15 * yesScale}px ${30 * yesScale}px`; // Fait grossir le padding aussi  
  
            // Bonus : change le texte si elle persiste  
            noClicks++;  
            if(noClicks > 3) question.innerText = "Please? 🥺";  
            if(noClicks > 7) question.innerText = "You're clicking the wrong one! 😤";  
            if(noClicks > 12) question.innerText = "Just click YES already! 🙏";  
        }  
  
        // Esquive au survol (PC) et au toucher (Mobile)  
        noBtn.addEventListener('mouseover', moveButton);  
        noBtn.addEventListener('touchstart', (e) => {  
            e.preventDefault(); // Empêche le défilement et permet le mouvement  
            moveButton();  
        });  
        ...
        yesBtn.addEventListener('click', () => {  
            document.getElementById('card').innerHTML = `  
                <img src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExbmZ0bmZ0bmZ0bmZ0bmZ0bmZ0bmZ0bmZ0bmZ0bmZ0bmZ0bmZ0&ep=v1_gifs_search&rid=giphy.gif&ct=g" style="max-width: 250px;">  
